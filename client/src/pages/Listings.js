...
const FILTERS = ['All', 'Villa', 'Homestay', 'Beach Villa', 'Bungalow', 'Luxury Camp', 'Chalet'];
      ...
      <div className="listings__modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="listings__modal-body">
          ...
          <div className="listings__modal-rating">
            <span>★</span> {listing.rating} ({listing.reviews} reviews)
          </div>
          <h2>{listing.name}</h2>
              ...
              <div className="listings__modal-platform-btns">
                {listing.platforms.map((p) => {
                  ...
                  return (
                    <span
                      key={p}
                      className="listings__modal-platform-tag"
                    >
                      {p}
                    </span>
                  );
...
export default function Listings() {
  ...
  useEffect(() => {
    ...
    if (activePlatform !== 'All') {
      filtered = filtered.filter((l) => Array.isArray(l.platforms) && l.platforms.includes(activePlatform));
    }
    if (sortBy === 'rating') {
                  ...
                  >
                    <div className="listing-card__image">
                      ...
                      <div className="listing-card__platforms">
                        {(listing.platforms || []).slice(0, 2).map((p) => {
                          const link = platformLinks[p];
                          if (link) {
                            return (
                              <a
