      ...
      <div className="home__card-body">
        <div className="home__card-rating">
          {rating ? (
            ...
            <span className="home__card-new-badge">New</span>
          )}
        </div>
        ...
        <div className="home__card-footer">
          <div>
            {p.price > 0 ? (
              <>
                <span className="home__card-price">₹{Number(p.price).toLocaleString()}</span>
                <span className="home__card-per"> / night</span>
              </>
            ) : (
              <span className="home__card-price-note">See price on Airbnb</span>
            )}
          </div>
          <Link to={`/listings/${p.hotelId}`} className="home__card-btn">View →</Link>
        </div>
      </div>
    </div>
  );
